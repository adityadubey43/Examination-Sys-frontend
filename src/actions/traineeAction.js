...
export const ValidateTheTestPassword = (password) => (dispatch) => {
...
export const fetchTraineeTestQuestions = (tid, subId) => (dispatch) => {
  Post({
...
export const fetchSubjectList = (tid) => (dispatch) => {
  Post({
    url: `${apis.FETCH_TRAINEE_TEST_SUBJECT}`,
    data: {
      id: tid,
    },
  })
    .then((response) => {
      ...
      if (response.data.success) {
        dispatch({
          type: "UPDATE_TRAINEE_TEST_SUBJECT",
          payload: response.data.data,
        });
      } else {
        Alert("error", "Error!", response.data.message);
...
export const fetchTraineeTestAnswerSheet = (tid, uid, subId) => (dispatch) => {
  Post({
